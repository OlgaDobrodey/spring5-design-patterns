package main.com.packt.patterninspring.chapter7.bankapp.callbacks;
	...
	public List<Account> extractData(ResultSet resultSet) throws SQLException, DataAccessException {
		...
		while (resultSet.next()) {
			if (extractedAccounts == null) {
				...
				account = new Account(resultSet.getLong("ID"), resultSet.getString("NAME"), resultSet.getLong("BALANCE"));
			}
			account = new Account(resultSet.getLong("ID"), resultSet.getString("NAME"),resultSet.getLong("BALANCE"));
